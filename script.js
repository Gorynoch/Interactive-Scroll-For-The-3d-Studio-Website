(function () {
  function initRainStyle() {
    ...
    function shapeLerp(shape1, shape2, t) {
      ...
      const result = s1.map((v, i) => lerp(v, s2[i], t));
    ...
    function update() {
      ...
      const blockCenters = games.map(g => {
      ...
      let lowerIndex = 0;
      for (let i = 0; i < blockCenters.length - 1; i++) {
        if (scrollTop + viewportHeight / 2 >= blockCenters[i]) lowerIndex = i;
      }
      let upperIndex = Math.min(lowerIndex + 1, blockCenters.length - 1);

      const segmentStart = blockCenters[lowerIndex];
      const segmentEnd = blockCenters[upperIndex];
      const t = segmentEnd - segmentStart === 0 ? 0 : (scrollTop + viewportHeight / 2 - segmentStart) / (segmentEnd - segmentStart);

      const color1 = games[lowerIndex].dataset.color || "#f88236";
      const color2 = games[upperIndex].dataset.color || "#f88236";
      ...
      const shape1 = games[lowerIndex].dataset.shape || "soft";
      const shape2 = games[upperIndex].dataset.shape || "soft";
      blob.style.borderRadius = shapeLerp(shape1, shape2, t);

      const title = t < 0.5 ? games[lowerIndex].dataset.title : games[upperIndex].dataset.title;
      ...
      games.forEach((g, i) => g.classList.toggle("rg-active", i === lowerIndex));
  ...
  const timer = setInterval(() => {
    ...
    if (tries > 50) {
      clearInterval(timer);
      console.log("RainStyle: init failed");
    }
  }, 200);
})();